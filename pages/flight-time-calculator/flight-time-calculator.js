...
Page({
  data: {
    ...
    flightTypes: [
      { label: '普通飞行', max24h: 4, max168h: 20 },
      { label: '转场飞行', max24h: 5, max168h: 20 }
    ],
    flightTypeIndex: 0,
    history: [],
    totalHours168: 0,
  ...
  updateStats() {
    ...
    // 计算颜色状态
    // 24小时限制：超过3小时橙色，达到或超过4小时红色
    let color24h = 'normal';
    if (totalHours24 >= 4) {
      color24h = 'danger';
    } else if (totalHours24 > 3) {
      color24h = 'warning';
    }
    
    // 168小时限制：超过19小时橙色，达到或超过20小时红色
    let color168h = 'normal';
    if (totalHours168 >= 20) {
      color168h = 'danger';
    } else if (totalHours168 > 19) {
      color168h = 'warning';
    }
  ...
  saveHistory() {
    ...
    wx.setStorageSync('flightHistory', history);
  },

  // 加载历史记录
  loadHistory() {
    try {
      const history = wx.getStorageSync('flightHistory') || [];
  ...
  onShareAppMessage() {
    return {
      title: '学员训练时间计算器',
      path: '/pages/flight-time-calculator/flight-time-calculator'
    };
  },
  ...
  goToTeacher() {
    wx.navigateTo({
      url: '/pages/flight-time-calculator-teacher/flight-time-calculator-teacher'
    });
  }
});